...
`sendInactivityCheckins()` runs immediately after, one day past that threshold (`INACTIVITY_DAYS + 1`). It queries `db.getMembersEligibleForCheckin()`, which reuses the same "no active AFK" exclusion but adds a second condition: the member either has no `member_checkin_dms` row at all, or their most recent one was resolved (`responded_text`/`responded_reaction`) and a *later* `member_snapshots` row shows them active again (`last_active` NOT matching the `Nd ago` pattern) -- so a member who checks in, comes back, then goes inactive again later gets a fresh DM rather than being permanently excluded.

For each eligible member, the DM (`utils/checkinContent.js`'s `CHECKIN_MESSAGE`) is sent with all four `CHECKIN_REACTIONS` emoji pre-added, and a `member_checkin_dms` row is inserted with `status='pending'`, `dm_message_id` set to the sent message's id (the only way to later match a reaction back to this specific row, since it lives in a DM channel outside any guild). A send failure inserts a `status='dm_failed'` row instead (no `dm_message_id`, since nothing was sent) and posts a note to `CHECKIN_RELAY_CHANNEL_ID` immediately, rather than waiting for a response that will never come.

**Rollout gate:** `CHECKIN_TEST_MODE_DISCORD_ID` (`bot_config`, admin-panel editable), when set, filters the eligible list down to just that one Discord id before any DM is sent -- this feature has no private-preview equivalent (unlike a channel post that can be checked in `bot-chatter` first), since it DMs real members directly. Clear it once the message content/behavior is verified live.

**Response capture** is split across two files because of a real ordering constraint. `index.js`'s `messageCreate` listener fires `handleTranslationRelay`/`handleAsk` as unawaited promises back-to-back -- so `checkinResponseHandler.js`'s `resolveCheckinReply(message)` is called *synchronously*, directly (not awaited, not wrapped in another `.catch()`-chained async call in that same list), before any of those async handlers start. It looks up a `pending` row by `message.author.id`, updates it to `responded_text`, and returns the resolved row (or `null`). This guarantees the row's status is already updated by the time `askHandler.js`'s own guard-check (`db.getRecentlyResolvedCheckin(message.author.id, 2000)`, a 2-second window specifically sized to only catch the *exact* message that just closed the check-in) reads it -- an async handler in that same fire-and-forget list would race the DB write against the guard's read, both starting at effectively the same instant.

Reactions are simpler: `handleCheckinReaction(reaction, user, client)` is wired into `messageReactionAdd` normally (async, `.catch()`-wrapped like every other handler there), since a reaction and the guard-check in `askHandler.js` don't share the same race -- reactions never touch `messageCreate` at all. It matches `reaction.message.id` against a `pending` `member_checkin_dms` row, ignores any reaction that isn't one of the four valid emoji (same fail-safe philosophy as `gloryctaReactionGuard.js` -- never act on something it can't positively identify), and otherwise resolves the row to `responded_reaction`.

Whichever happens first wins: both `resolveCheckinReply` and `handleCheckinReaction` query specifically for `status='pending'`, so once one resolves the row, the other's lookup simply finds nothing on any later attempt -- no explicit "already handled" bookkeeping needed.

`postCheckinRelayAndConfirm(row, {user, responseLine})` (shared by both paths) posts an embed to `CHECKIN_RELAY_CHANNEL_ID` and DMs the member a short confirmation. The member's name/mention is built by an internal `checkinDisplayName(row)` and placed in the embed's **description**, never the title -- confirmed live that Discord does not render mentions or markdown inside embed titles at all, so a mention in `setTitle()` shows as literal raw text (`<@123...>`) instead of a clickable ping.

`askHandler.js`'s system prompt also gets a soft, non-hardcoded nudge: if the sender has a check-in resolved within the last 5 minutes (a wider window than the 2s guard above -- this one is guidance text, not a hard gate), a note is added asking the model to gently mention that only their first message after the check-in went to the team, and point them to the main server if they want to share more. `docs/bot-guide.md` carries the member-facing explanation of the same rule.
