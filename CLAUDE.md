...
- `translation_relay_messages` · one row per relayed copy of a BATCH, INCLUDING the source (not one row per message — a batch of several same-author messages collapses to a single row per channel) · id, relay_group_message_id (shared across every copy of the same logical batch — the source row's own `id`), channel_id, message_id (UNIQUE · the batch's first message id for the source row, the sent webhook message id for target rows), author_id, author_display_name, language, text (all batched lines joined), `batch_message_ids` (JSON array of every original Discord message id folded into this batch, used to resolve a reply that targets any line, not just the first), `last_line_text` (just the final batched line, translated for that row's language · used for reply-quote prefixes so a multi-line batch quotes only its last line, not the whole batch) · used to resolve reply-quoting (look up what a reply references, then find that logical batch's copy in the replying channel's own language)
- `translation_usage` · id, message_id, input_tokens, output_tokens, target_count · one row per successful Claude translation call (not logged on failure, one call per BATCH so this is now per-batch cost, keyed on the batch's first message_id) · exists so real per-batch translation cost can be measured after a trial period instead of estimated
