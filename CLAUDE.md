...
| `utils/checkinContent.js` | Pure constants for the day-4 inactivity check-in DM · `CHECKIN_MESSAGE` (the DM template) and `CHECKIN_REACTIONS` (`{emoji: meaningText}`, 4 entries: 👍 still playing/🔽 wants to step down/💤 taking a break/👋 done playing) · shared by `scan.js` (sending) and `checkinResponseHandler.js` (relaying), so the meaning text is defined exactly once |
| `utils/handlers/checkinResponseHandler.js` | Owns both halves of check-in response capture · `handleCheckinReaction` (async, `messageReactionAdd`) matches a reaction's message against a `pending` `member_checkin_dms` row, ignores anything that isn't one of the 4 valid emoji · `resolveCheckinReply` (**synchronous, no `await`**, called directly — not awaited — from `index.js`'s `messageCreate` listener BEFORE any async handler starts) looks up and updates the row only, returning it or `null` · this split exists because `index.js` fires `handleTranslationRelay`/`handleAsk` as unawaited promises back-to-back, so an async check-in handler in that same list would race `askHandler.js`'s own guard-check read against this handler's DB write, both starting at effectively the same instant · `postCheckinRelayAndConfirm(row, {user, responseLine})` posts the relay embed (to `CHECKIN_RELAY_CHANNEL_ID`) and sends the confirmation DM · the member mention/name (`checkinDisplayName`, internal) always goes in the embed **description**, never the title — Discord does not render mentions or markdown inside embed titles, confirmed live (a mention placed in `setTitle()` showed as literal raw text) · confirmed live: reacting immediately after the bot's own rapid-fire self-reactions (4 emoji added in a tight loop when the DM sends) can silently fail to register the first human reaction — a real member reading the DM first before reacting won't hit this in practice, not code-fixed |
...
| `/scan` | Runs Python scraper, posts results · then posts inactivity alert (active members only, AFK excluded) · then sends day-4 check-in DMs (one day past the alert threshold, once per absence streak — see `member_checkin_dms`) · authorized user only |
...
- `member_checkin_dms` · one row per day-4 inactivity check-in DM sent · member_id, discord_id, `dm_message_id` (matches a later reaction back to this row — `NULL` only for a `dm_failed` row, since no message was ever sent), sent_at, days_inactive_at_send, `status` (`pending`/`responded_text`/`responded_reaction`/`dm_failed`), response_text, response_emoji, responded_at · a member can have many rows over time (one per absence streak, not collapsed) — the eligibility query (`db.getMembersEligibleForCheckin`) treats a member as freshly eligible again once a LATER `member_snapshots` row shows them active, even if an older row already resolved · repointed (not collision-collapsed) by `mergeMembers`, same shape as `member_notes`/`member_name_history`
...
| `CHECKIN_RELAY_CHANNEL_ID` | *(set via admin panel)* | — |
...
- Check-in DM response capture (`checkinResponseHandler.js`'s `resolveCheckinReply`) is deliberately SYNCHRONOUS and called before any async handler starts in `index.js`'s `messageCreate` listener — `handleTranslationRelay`/`handleAsk` are fired as unawaited promises back-to-back there, so an async check-in handler in that same list would race `askHandler.js`'s own guard-check read against this handler's DB write, both starting at effectively the same instant. Do not "clean this up" into another `.catch()`-wrapped async call in that list — it would reintroduce the race.
- Discord embed **titles** do not render mentions or markdown — only the description and field values do. A member mention placed in `setTitle()` shows as literal raw text (`<@123...>`) instead of a clickable ping, confirmed live 2026-08-25 building the check-in feature. Any future embed needing a mention/formatted content must put it in the description or a field, never the title.
- SQLite `datetime('now')` (space-separated: `YYYY-MM-DD HH:MM:SS`) and JS `new Date().toISOString()` (T-separated with milliseconds: `YYYY-MM-DDTHH:MM:SS.sssZ`) are NOT safely comparable via a plain string `>`/`<` in SQL — the character difference at the separator (space, ASCII 32, sorts before `T`, ASCII 84) can make a genuinely later `datetime('now')` timestamp compare as *earlier* than an ISO one. Real production timestamps here are consistent (the miner's `scraped_at` and the bot's own `sent_at`/`responded_at` are all written via `toISOString()`-family calls), so this only bit a synthetic test snapshot inserted via `datetime('now')` while verifying `getMembersEligibleForCheckin`'s new-absence rule — but it's a real trap for any future ad-hoc test data or manual DB insert that mixes the two formats.
