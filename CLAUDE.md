...
- Theme system: DaisyUI v5 + Tailwind v4 · BOTH sites now have a full Vite build pipeline (admin migrated 2026-06-19) · admin bundles DaisyUI + `shared/theme.css` via Vite, so the old `/daisyui.css` + `/shared` Express routes and the standalone `theme-demo.html` were removed · admin CSP keeps `'unsafe-inline'` in scriptSrc only for the FOUC theme-init `<script>` in `<head>`
